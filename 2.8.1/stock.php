...
// SQL to create 'hashowner' table
$hashstock_table_sql = "
CREATE TABLE IF NOT EXISTS `hashowner` (
...
if (!$conn->query($hashstock_table_sql)) {
    die("Error creating hashowner table: " . $conn->error);
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if ($user_id <= 0) {
        ...
        if (empty($filehash) || empty($extension)) {
            ...
            $stmt_check = $conn->prepare("SELECT deposit FROM hashowner WHERE filehash = ? ORDER BY date DESC LIMIT 1");
            ...
            if ($can_insert) {
                ...
                try {
                    ...
                    $stmt_insert = $conn->prepare("INSERT INTO hashowner (filehash, extension, deposit, date, user_id) VALUES (?, ?, ?, NOW(), ?)");
