...
class Blockchain {
    ...
    private function saveBlock(Block $block, string $filehash): void {
        $path = $this->getStoragePath($filehash);
        $filename = $filehash .'.txt';
        file_put_contents('hash/' . $filename , json_encode($block->getBlockData(), JSON_PRETTY_PRINT));
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($error)) {
        ...
        $block = $blockchain->createBlock($blockData, $blockData['filehash']);
        //$blockPath = 'hash/' . $blockData['category'] . '/' . ($blockchain->getChainLength() - 1) . '.txt';
        $blockPath = 'hash/' . $blockData['filehash'] . '.txt';
