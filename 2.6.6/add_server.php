...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['url'])) {
    ...
    if (isUrlOnline($url)) {
        // Append URL to servers.txt using fopen
        $file = fopen(SERVERS_FILE, 'a'); // Open file in append mode
        if ($file === false) {
            echo json_encode(['success' => false, 'message' => 'Failed to write URL to servers.txt. Check permissions.']);
            exit;
        }
        
        // Write the URL with a newline
        if (fwrite($file, PHP_EOL . $url) === false) {
            fclose($file);
            echo json_encode(['success' => false, 'message' => 'Failed to write URL to servers.txt. Check permissions.']);
            exit;
        }
        ...
        // Create the hash marker files in hash_check directory using fopen
        foreach ($hashFiles as $file) {
